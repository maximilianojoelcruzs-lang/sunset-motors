...
/**
 * El catálogo, y **si lo que devuelve es la semilla del código** (`semilla: true`).
 *
 * El dato viaja dentro porque salía de un `esSemilla()` aparte que volvía a leer la misma
 * colección: el editor pedía el catálogo dos veces para pintar una frase.
 */
export async function obtener() {
  const guardado = await leer(CATALOGO, null);
  if (guardado?.secciones?.length) return { ...guardado, semilla: false };
  return {
...
export async function reemplazar(secciones, usuario) {
  ...
  await cambiar(CATALOGO, () => ({ lista: catalogo, valor: { catalogo } }), { porDefecto: null });
  return { catalogo };
}
