...
export async function soloCasino(usuario) {
  ...
  const buscado = usuario.trim().toLowerCase();
  const ficha = (await listarUsuarios()).find((u) => u.usuario.toLowerCase() === buscado);
  return Boolean(ficha?.casino) && !ficha?.admin;
}
...
export async function cambiarCasino(usuario, casino) {
  ...
  const copia = [...lista];
  copia[i] = { ...copia[i], casino: Boolean(casino) };
...
export async function crearUsuario(usuario, clave, admin = false, casino = false) {
  ...
  const ficha = { ...(await fichaNueva(limpio, clave, admin)), casino: Boolean(casino) };
