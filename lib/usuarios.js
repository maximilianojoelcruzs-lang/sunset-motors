...
export async function accesosDe(usuario) {
  if (typeof usuario !== 'string') return { admin: false, casino: false, taller: false };
  const buscado = usuario.trim().toLowerCase();
  const ficha = (await listarUsuarios()).find((u) => u.usuario.toLowerCase() === buscado);
  if (!ficha) return { admin: false, casino: false, taller: false };
...
async function cambiarBandera(usuario, bandera, valor) {
  const buscado = usuario.trim().toLowerCase();
  const lista = await listarUsuarios();
  const i = lista.findIndex((u) => u.usuario.toLowerCase() === buscado);
  if (i === -1) return { error: 'Ese usuario no existe.' };
  ...
  const copia = [...lista];
  copia[i] = { ...copia[i], [bandera]: Boolean(valor) };
  await guardar(USUARIOS, copia);
  return { ok: true };
}
...
export async function cambiarClave(usuario, clave) {
  ...
  // De la ficha nueva solo interesan las credenciales; el resto de la persona ya está.
