...
export async function sesionDeTaller() {
  ...
  // La cuenta ya no está, o está suspendida: al login, no al casino. Mandarla al casino sería
  // un rebote infinito, porque de allá tampoco puede entrar.
  if (!puertas.existe || puertas.suspendida) redirect('/login');
...
async function portero(puerta) {
  ...
  // Suspendida es 401 y no 403: la pantalla trata el 401 como «vuelve a entrar», y ahí el
  // login le dice con esas palabras que la cuenta está suspendida.
  if (puertas.suspendida) return corte('Esta cuenta está suspendida.', 401);
