...
/**
 * Guarda una imagen y devuelve su ruta interna (no una URL: la ruta sola no sirve para
 * verla). `carpeta` separa los usos, por ejemplo 'devoluciones' o 'flyers'.
 */
export async function guardarImagen(bytes, tipoDeclarado, carpeta) {
  if (bytes.byteLength > MAX_BYTES) {
    return { error: `La imagen pesa más de ${Math.round(MAX_BYTES / 1024 / 1024)} MB.` };
  }

  // Manda el contenido, no la etiqueta. Si no calzan, el archivo no es lo que dice ser.
  const tipo = tipoReal(bytes);
  if (!tipo) return { error: 'Ese archivo no es una imagen JPG, PNG ni WEBP.' };
