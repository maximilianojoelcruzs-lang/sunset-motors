...
export async function aplicar(usuario, filas) {
  ...
  for (const fila of buenas) {
    ...
    porId.set(antes.id, {
      ...
      sugerido: sugerenciaDe(fila) || antes.sugerido || '',
      cantidad,
      visto: cuando,
      vistoPor: usuario,
    });
...
function sugerenciaDe(fila) {
  const propuesta = normalizarNombre(fila.sugerencia ?? '');
  if (!propuesta) return '';
  const corto = partesNombre(nombreLeido(fila));
