...
export async function aplicar(usuario, filas) {
  ...
  for (const fila of buenas) {
    ...
    if (antes.cantidad !== fila.cantidad) cambiados += 1;
    // Se conserva el `id` y el nombre que ya tenía: si alguien completó a mano un nombre que
    // el juego enseña cortado, una captura nueva no puede volver a truncarlo.
    porClave.set(clave, { ...antes, cantidad: fila.cantidad, visto: cuando, vistoPor: usuario });
  }

  await guardar(INVENTARIO, [...porClave.values()]);
