...
import { exigirEnlace } from './enlaces.js';
...
/**
 * Publica un flyer. La imagen es **un enlace pegado**, no un archivo subido.
 *
 * Antes se subía desde el escritorio y quedaba en el bucket privado. Se cambió a pedido del
 * usuario: la imagen ya está publicada en algún sitio —la hace el diseñador y la comparte por
 * Discord—, así que bajarla y volver a subirla era un paso de más. Los flyers viejos siguen
 * teniendo su `imagen` guardada y se sirven igual por `/api/flyers/:id/imagen`.
 */
export async function crearFlyer(usuario, { titulo, enlace }) {
  if (typeof titulo !== 'string' || !titulo.trim()) return { error: 'Ponle un título.' };

  const { enlace: limpio, error: malEnlace } = exigirEnlace(enlace);
