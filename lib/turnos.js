...
import { TURNOS, cambiar, leer, modificar } from './almacen.js';
...
export async function turnoAbierto(usuario) {
  ...
  return abiertoEn(lista, usuario);
}

/**
 * El turno abierto dentro de una lista **que ya se leyó**. Es puro: no toca el almacén.
 *
 * El panel pedía el registro entero y después `turnoAbierto()`, que vuelve a leerlo —y encima
 * por `leerAlDia()`, que puede escribir para cerrar lo vencido. Con la lista en la mano, el
 * turno propio es un `find`.
 */
export function abiertoEn(lista, usuario) {
...
export async function corregir(id, cambios, porQuien) {
  return cambiar(TURNOS, (todos) => {
    ...
    return { lista: copia, valor: { turno: copia[i] } };
  });
}

export async function borrar(id) {
  const { ok } = await cambiar(TURNOS, (todos) => {
    const quedan = todos.filter((t) => t.id !== id);
    if (quedan.length === todos.length) return { error: 'Ese turno no existe.' };
    return { lista: quedan, valor: { ok: true } };
  });
  return Boolean(ok);
}
