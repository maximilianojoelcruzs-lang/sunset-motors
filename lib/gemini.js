...
6. Además, en "sugerencia", escribe el nombre completo más probable del objeto **mirando su
   foto**, no solo el texto. Es un taller mecánico de un juego de rol: son repuestos y
   herramientas. La foto es lo que distingue dos casillas con el mismo texto cortado: si una
   muestra una caja de herramientas y otra unas palancas de neumático, son "KIT DE REPARACIÓN"
   distintos. La sugerencia debe EMPEZAR igual que el nombre, letra por letra; solo añade lo que
   falta. Si el nombre no está cortado, repite el nombre.
7. Cuando DOS casillas muestren el mismo texto cortado, míralas con especial cuidado: fíjate en
   la foto y en el peso de cada una para no cambiarlas de sitio. Una caja de herramientas grande
   y pesada no es lo mismo que unas palancas de neumático ligeras.`;
...
const ESQUEMA = {
  ...
  items: {
    ...
    properties: {
      ...
      peso: { type: 'STRING' },
      // El nombre entero deducido de la foto. **Nunca se usa para casar** —para eso está
      // `nombre`, literal y estable— sino para proponerlo en el panel de nombres. Si se casara
      // por aquí, dos capturas darían dos nombres y volverían los duplicados.
