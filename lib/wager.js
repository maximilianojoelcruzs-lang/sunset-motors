...
/**
 * Arranca el contador con lo que haya en el registro de jugadas.
 *
 * Es un apaño de una sola vez, para no empezar el primer ciclo con todo a cero teniendo un
 * historial delante. **Es parcial y hay que decirlo**: `sunset:jugadas` guarda las 500 últimas,
 * así que lo anterior a eso no está y no hay forma de recuperarlo.
 */
export async function sembrarDesdeJugadas() {
  const { JUGADAS } = await deFichas();
  const jugadas = await leer(JUGADAS, []);

  const totales = {};
  const cuenta = {};
  for (const j of jugadas) {
    const apuesta = Math.round(Number(j.apuesta));
    if (!j.usuario || !Number.isFinite(apuesta) || apuesta <= 0) continue;
    totales[j.usuario] = (totales[j.usuario] ?? 0) + apuesta;
    cuenta[j.usuario] = (cuenta[j.usuario] ?? 0) + 1;
  }

  const masAntigua = jugadas.at(-1)?.cuando;
  await modificar(
    WAGER,
    () => ({
      lista: { desde: masAntigua ?? new Date().toISOString(), totales, jugadas: cuenta },
      hecho: true,
    }),
    () => true,
    { porDefecto: vacio() }
  ...
  return { usuarios: Object.keys(totales).length, jugadas: jugadas.length };
}
