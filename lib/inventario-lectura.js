...
export function comparar(leidos, actuales) {
  ...
  // Qué textos cortados salen más de una vez en esta tanda. **En esos no se aplica la deducción
  // sola**: con dos casillas que se leen igual, el lector puede cruzarlas —se vio: la caja de
  // herramientas recibió el nombre de la rueda—, y eso hay que mirarlo. Los demás entran ya
  // completos, que es lo que se espera al escanear.
  ...
  for (const crudo of leidos) {
    ...
    const fila = !antes
      ? {
          ...
          nombre:
            (!repetidos.has(clave) && completar(nombre, crudo.sugerencia)) || nombre,
          leido: nombre,
