...
/**
 * La deducción del lector, **solo si empieza igual** que el nombre cortado.
 *
 * Empezar igual es lo que hace que el nombre guardado siga casando por prefijo con la captura
 * cortada de mañana, y por eso no puede crear duplicados.
 *
 * **Vale también cuando no añade letras.** Antes se exigía que alargara el nombre, y entonces
 * «GRÚA DE MOTOR…» se quedaba con los puntos para siempre: el lector respondía «GRÚA DE MOTOR»
 * —que es el nombre entero, los puntos son cosa de cómo lo dibuja el juego— y al no añadir nada
 * se descartaba. Aceptarlo quita los puntos, que es lo único que sobraba.
 */
export function completar(literal, sugerencia) {
  const propuesta = normalizarNombre(sugerencia ?? '');
  if (!propuesta) return '';
  const corto = partesNombre(literal);
  const larga = partesNombre(propuesta);
  // `base` sirve para comparar, **no para guardar**: va sin tildes ni ñ. Devolver eso dejaba
  // «GRUA DE MOTOR» y «PANO» en el inventario. La propuesta ya viene sin puntos —se comprobó
  // arriba que no está truncada—, así que se devuelve tal cual.
  return larga.truncado || !larga.base.startsWith(corto.base) ? '' : propuesta;
}
...
export function comparar(leidos, actuales) {
  ...
  // Qué textos cortados salen más de una vez en esta tanda. La deducción se aplica igual —los
  // nombres a medias no sirven de nada—, pero esas filas salen **marcadas para revisar**: con
  // dos casillas que se leen igual el lector puede cruzarlas, y se vio pasar. La tabla de
  // confirmación enseña las dos con su cantidad al lado, que es donde se nota el cambiazo.
  const repetidos = new Set();
  const yaVisto = new Set();
  for (const c of leidos) {
    const b = partesNombre(c.nombre).base;
    if (!b) continue;
    if (yaVisto.has(b)) repetidos.add(b);
    yaVisto.add(b);
  }
  ...
  for (const crudo of leidos) {
    ...
    const fila = !antes
      ? {
          // Se guarda ya completo cuando la deducción es fiable. Casar sigue usando `leido`,
          // que es el texto literal de la captura, así que esto no puede crear duplicados.
          nombre: completar(nombre, crudo.sugerencia) || nombre,
          leido: nombre,
          // Dos casillas con el mismo texto cortado: el nombre viene de la foto y conviene
          // mirarlo antes de guardar.
          revisar: repetidos.has(clave) && Boolean(completar(nombre, crudo.sugerencia)),
